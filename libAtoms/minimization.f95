...
module minimization_module
  ...
  public :: precon_data, preconminim, precondimer
  ...
  integer :: n_func_eval = 0, n_grad_eval = 0, n_both_eval = 0
  ...
  function minim(x_in,func,dfunc,method,convergence_tol,max_steps, linminroutine, hook, hook_print_interval,  &
    ...
    if(normsqgrad_f .LT. convergence_tol)then  
       ...
       call print(trim(method)//" iter = "// 0 //" df^2 = " // normsqgrad_f // " f = " // f &
            &// " "//lsteps//" linmin steps eps = "//eps//' '// &
	      ' n_func deriv both evals '//n_func_eval//' '//n_grad_eval//' '//n_both_eval, PRINT_VERBOSE)
       exit_flag = 1
    end if
    ...
    do while((main_counter .LT. max_steps) .AND. (.NOT.(exit_flag.gt.0)))
	  ...
	  dumbool=test_gradient(x, func, dfunc,data=data)
       ...
       if(convergence_counter == convergence_window) then
          ...
          call print(trim(method)//" iter = " // main_counter // " df^2= " // normsqgrad_f // " f= " // f//' '// &
	   ' n_func deriv both evals '//n_func_eval//' '//n_grad_eval//' '//n_both_eval)
  ...
  function preconminim(x_in,func,dfunc,build_precon,pr,method,convergence_tol,max_steps,efuncroutine,LM, linminroutine, hook, hook_print_interval, am_data, status,writehessian,gethessian,getfdhconnectivity)
   ...
   character(len=1),optional, intent(in) ::data(:)
    ...
    if (trim(method) == 'preconCG') then
      ...
      call print('Unrecognized minim method '//trim(method)//', exiting')
      call exit()
    end if
...
!end if
	  ...
	  n_both_eval = n_both_eval + 1
          ...
          call verbosity_pop()
#endif
          call print(trim(method)//" iter = 0 f = "//f// ' |g|^2 = '// normsqgrad// &
	      ' n_func deriv both evals '//n_func_eval//' '//n_grad_eval//' '//n_both_eval, PRINT_NORMAL)
        end if  
        ...
        elseif (doGHFDH) then
            ...
            call GLTR_terminate( GLTR_data, GLTR_control, GLTR_inform) 
#else /* HAVE_GALAHAD */
        elseif (doGHLBFGS .or. doGHLSR1 .or. doGHFD .or. doGHFDH) then
	  call System_abort("Invalid minimization method - no GALAHAD support")
#endif /* HAVE_GALAHAD */
