...
if opt.offset is not None:
    if len(opt.offset) != len(args):
        p.error('Number of -O/--offset options must match '
...
sources = [AtomsReader(arg, start=offset, range=[atoms_start, atoms_stop])
...
for configs in zip(*sources):
    ...
    for at in configs:
        ...
        for i, j, k in angles:
            ...
            u_ij = u_ij/u_ij.norm()
            u_ik = u_ik/u_ik.norm()
            angle = np.arccos(np.dot(u_ij, u_ik))
            angles.append(angle)
