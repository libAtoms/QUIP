...
class PyDescShell():
    ...
    _exponent_regex = "exponents\s?=\s?.((\s?(-\d+)\s?)+)." # RegEx to find the dot_product exponents from command line args
    _exponent_regex = re.compile(_exponent_regex)
    def __init__(self, desc_xml):
        self.weights = np.array([float(child.attrib["alpha"]) for child in desc_xml if "sparseX" in child.tag])
        self.sparse_cuts = np.array([float(child.attrib["sparseCutoff"]) for child in desc_xml if "sparseX" in child.tag])
        ...
        if self._int_cov_type == 1: # ard_se
            self.cov_prop = float([child.text.split()[0] for child in desc_xml if "theta" in child.tag][0])
        ...
        _desc_children = [child for child in desc_xml if "descriptor" in child.tag]
        ...
        self.name = "".join([chemical_symbols[int(Z)] for Z in self.Zs]) + " " + self.desc_type
...
class PyGAPShell():
    ...
    def __init__(self, xml, mean_weights = None):
        ...
        for item in iso_atom_energies:
            self.isolated_atom_energies[int(item.attrib["Z"])] = float(item.attrib["value"])
        ...
        self.descriptors = [PyDescShell(desc_xml) for desc_xml in descriptors]
    ...
    def _posterior_sample(self):
        ...
        if self.R is not None:
            ...
            raise FileNotFoundError(f"R matrix not found. {self.R_fname} does not exist")
    ...
    def _xml_sample(self):
        ...
        for i in range(self.num_desc):
            ...
            desc_wts = [child for child in new_descs[i] if "sparseX" in child.tag]
    ...
    def draw_posterior_sample(self):
        return PyGAPShell(self._xml_sample(), mean_weights = self.mean_weights)


def read_xml(path_to_xml):

    gap = PyGAPShell(parse(path_to_xml))
        
    R_fname = path_to_xml + ".R." + gap.gap_label

    if os.path.exists(R_fname):
        gap.R = np.loadtxt(R_fname).reshape((gap.total_nsparse, gap.total_nsparse)).T
    else:
        gap.R = None
