...
class TestConvert_UnsupportedInfo(QuippyTestCase):

    def setUp(self):
        from datetime import datetime
        from ase.spacegroup import crystal
        # creating a crystal puts a Spacegroup() in the info dict
        self.fcc_al = crystal('Al', [[0, 0, 0]], spacegroup=225, cellpar=4.05)
        # Other random class that gets repr'd
        self.now = datetime.now()
        self.fcc_al.info['timestamp'] = self.now

    def test_info_custom_types(self):
        conv_atoms = quippy.Atoms(self.fcc_al)
        self.assertEqual(conv_atoms.info['timestamp'], repr(self.now))
        self.assertEqual(conv_atoms.info['spacegroup'], 'F m -3 m')
