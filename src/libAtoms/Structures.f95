...
contains
  ...
  subroutine disloc_noam(at, p, l, b, close_threshold)
	...
	theta = atan2(delta_p_rot(2), delta_p_rot(1))
      ...
      call print("atom " // i_at // " pos " // at%pos(:,i_at) // " delta_p " // delta_p // " theta " // theta, PRINT_INVESTIGATE)
...
!    real(dp) :: cutoff
    ...
    call print("find_motif", verbosity=PRINT_INVESTIGATE)
    call print(motif, verbosity=PRINT_INVESTIGATE)
    if (present(mask)) then
      call print("mask", verbosity=PRINT_INVESTIGATE)
      call print(mask, verbosity=PRINT_INVESTIGATE)
    endif
...
!
    ...
    call print("opt_atom " // opt_atom // " Z " // Z(opt_atom), verbosity=PRINT_INVESTIGATE)
    ...
    do i = my_start, my_end
       ...
       call print("check atom " // i // " Z " // at%Z(i), verbosity=PRINT_INVESTIGATE)

       if (present(mask)) then
          if (.not.mask(i)) then
	    call print("   i not in mask, skipping", verbosity=PRINT_INVESTIGATE)
	    cycle
	   endif
       end if
	  ...
	  call print("   i has wrong Z to match opt_atom, skipping", verbosity=PRINT_INVESTIGATE)
	  ...
	  call print("   i has incompatible neighbour list, skipping", verbosity=PRINT_INVESTIGATE)
          cycle
       end if
	 ...
	 call select(core, core_raw, mask(int_part(core_raw,1)))
       ...
       if (core%N < N) then
	  call print("   i's cluster is too small, skipping", verbosity=PRINT_INVESTIGATE)
       ...
       if (.not.match) then
	  call print("   i's cluster has mismatching atom numbers, skipping", verbosity=PRINT_INVESTIGATE)
	  ...
	  do ji=1, n_neighbours(at, k, alt_connect=alt_connect)
       ...
       call print("core", verbosity=PRINT_INVESTIGATE)
       call print(core, verbosity=PRINT_INVESTIGATE)
       call print("adjacency mat", verbosity=PRINT_INVESTIGATE)
       call print(B, verbosity=PRINT_INVESTIGATE)
	  ...
	  call print("   i's cluster isn't actually keep enough, skipping", verbosity=PRINT_INVESTIGATE)
    ...
    !XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	  ...
	  call print("   i has some neighbor that doesn't match, skipping", verbosity=PRINT_INVESTIGATE)
          deallocate(M0,B)
	  ...
	  call print("  check permutation loop start", verbosity=PRINT_INVESTIGATE)
		...
		call print("  found match, indices " // match_indices, verbosity=PRINT_INVESTIGATE)
	     endif
	     deallocate(match_indices)

	     if (.not. do_find_all_possible_matches) then
	       call print("  not looking for _all_ matches, finished", verbosity=PRINT_INVESTIGATE)
	       exit
	     endif
          end if ! match
	     ...
	     call print("   i has no more premutations, leaving loop", verbosity=PRINT_INVESTIGATE)
