...
contains
...
#ifdef DEBUG
    if(current_verbosity() >= PRINT_INVESTIGATE) then
       call print('Entering test_form_bond, i = '//i//' j = '//j, PRINT_INVESTIGATE)
       call print('use_uniform_cutoff = '//use_uniform_cutoff, PRINT_INVESTIGATE)
       call print('cutoff = '//cutoff, PRINT_INVESTIGATE)
    end if
#endif
...
#ifdef DEBUG
	if (current_verbosity() >= PRINT_INVESTIGATE) call print('test_form_bond had check_for_dup=T, found bond already in table', PRINT_INVESTIGATE)
#endif
...
#ifdef DEBUG
    if(current_verbosity() >= PRINT_INVESTIGATE)  call print('d = '//d, PRINT_INVESTIGATE)
#endif
...
#ifdef DEBUG
    if(current_verbosity() >= PRINT_INVESTIGATE) call print('Leaving test_form_bond', PRINT_INVESTIGATE)
#endif
  ...
  function test_break_bond(this, cutoff_break, use_uniform_cutoff, Z, pos, lattice, i,j, shift, error)
...
#ifdef DEBUG
    if(current_verbosity() >= PRINT_INVESTIGATE) then
       call print('Entering test_break_bond, i = '//i//' j = '//j, PRINT_INVESTIGATE)
       call print(" cutoff_break = "// cutoff_break, PRINT_INVESTIGATE)
       call print(" use_uniform_cutoff ="//use_uniform_cutoff, PRINT_INVESTIGATE)
    end if
#endif
    ...
    d = norm(pos(:,j)+(lattice .mult. shift) - pos(:,i))
#ifdef DEBUG
    if(current_verbosity() >= PRINT_INVESTIGATE)  call print('d = '//d//' cutof = '//cutoff//' i = '//i//' j = '//j, PRINT_INVESTIGATE)
#endif
    if (d > cutoff) then
#ifdef DEBUG
       if(current_verbosity() >= PRINT_INVESTIGATE) call print('removing bond from tables', PRINT_INVESTIGATE)
#endif
...
#ifdef DEBUG
    if(current_verbosity() >= PRINT_INVESTIGATE) call print('Leaving test_break_bond', PRINT_INVESTIGATE)
#endif
  ...
  subroutine connection_calc_dists(this, at, parallel, error)
   ...
   subroutine get_min_max_images(is_periodic, cellsNa, cellsNb, cellsNc, cell_image_Na, cell_image_Nb, cell_image_Nc, i, j, k, do_i, do_j, do_k, &
	 ...
	 if (is_periodic(3)) then
      ...
      if (current_verbosity() >= PRINT_INVESTIGATE) then
         call print('get_min_max_images cell_image_Na min='//min_cell_image_Na//' max='//max_cell_image_Na, PRINT_INVESTIGATE)
         call print('get_min_max_images cell_image_Nb min='//min_cell_image_Nb//' max='//max_cell_image_Nb, PRINT_INVESTIGATE)
         call print('get_min_max_images cell_image_Nc min='//min_cell_image_Nc//' max='//max_cell_image_Nc, PRINT_INVESTIGATE)
      end if
