...
!             call print("d = " // d // ", i = " // i // ", s = " // s // ", lower = " // this%lower(d) // ", upper = " // this%upper(d))
    ...
    if (at%Ndomain /= last_N) then
       call print("DomainDecomposition : Repointing atoms object", PRINT_ANALYSIS)
...
!          call print("n = " // (n_recv/this%atoms_buffer_size), PRINT_ALWAYS)
    ...
    call print("DomainDecomposition : Sorting atoms object", PRINT_ANALYSIS)
...
!          RAISE_ERROR("Could not find key '" // keys%keys(i) // "'.", error)
          call print("DomainDecomposition : WARNING - Could not find key '" // keys%keys(i) // "', this property will not be communicated.", PRINT_ANALYSIS)
       else
