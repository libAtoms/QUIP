...
!$ use omp_lib
  ...
  integer,parameter::PRINT_INVESTIGATE    =   10000 ! aleph1
  ...
  !% range from 'ERROR' through 'NORMAL', 'VERBOSE', 'NERD' and 'INVESTIGATE'.
...
!$OMP end parallel
  ...
  function verbosity_of_str(str) result(val)
    ...
    if (trim(str) == 'ERROR') then
       ...
       val = PRINT_NERD
    else if (trim(str) == 'INVESTIGATE') then
       val = PRINT_INVESTIGATE
    else
