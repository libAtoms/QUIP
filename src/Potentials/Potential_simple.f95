...
module Potential_simple_module
  ...
  use error_module
  use system_module, only : dp, inoutput, PRINT_VERBOSE, PRINT_NERD, PRINT_ALWAYS, PRINT_INVESTIGATE, current_verbosity, INPUT, optional_default, parse_string, verbosity_push, verbosity_pop
    ...
    if (little_clusters) then
	 ...
	 if (do_rescale_E)  at_force_ptr = at_force_ptr*E_scale
	  ...
	  call set_value(params, "energy", "fd_energy")
          new_args_str = write_string(params, real_format='f16.8')
          call finalise(params)
          call print("calc_force="//trim(calc_force), PRINT_INVESTIGATE)
          call print("New_args_str: "//new_args_str, PRINT_INVESTIGATE)
