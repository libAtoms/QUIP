  ...
  recursive subroutine Potential_Sum_Calc(this, at, args_str, error)
    ...
    real(dp) :: energy, virial(3,3), dipole(3)
    real(dp), pointer :: at_force_ptr(:,:), at_local_energy_ptr(:), at_local_virial_ptr(:,:), at_local_dipole_ptr(:,:)

    real(dp) :: my_e_1, my_e_2
    real(dp), allocatable :: my_local_e_1(:)
    real(dp), allocatable :: my_f_1(:,:), my_local_virial_1(:,:), my_local_dipole_1(:,:)
    real(dp) :: my_virial_1(3,3), my_dipole_1(3)
    type(Dictionary) :: params
    character(STRING_LENGTH) :: calc_energy, calc_force, calc_local_energy, calc_virial, calc_local_virial, &
       calc_dipole, calc_local_dipole, calc_args_pot1, calc_args_pot2, my_args_str
    ...
    call param_register(params,"dipole", "", calc_dipole, help_string="Whether to force calculation of the dipole.")
    ...
    call param_register(params,"local_dipole", "", calc_local_dipole, help_string="Whether to force calculation of the local dipole.")
    ...
    if (len_trim(calc_dipole) > 0) then
       call get_param_value(at, trim(calc_dipole), my_dipole_1)
       if (store_contributions) call set_param_value(at, trim(calc_dipole)//"_pot1", my_dipole_1)
    endif
    ...
    if (len_trim(calc_local_dipole) > 0) then
       call assign_property_pointer(at, trim(calc_local_dipole), at_local_dipole_ptr, error=error)
       PASS_ERROR(error)
       allocate(my_local_dipole_1(3, at%N))
       my_local_dipole_1 = at_local_dipole_ptr
       if (store_contributions) call add_property(at, trim(calc_local_dipole)//"_pot1", at_local_dipole_ptr, overwrite=.true.)
    endif
    ...
    if (len_trim(calc_dipole) > 0) then
       call get_param_value(at, trim(calc_dipole), dipole)
       if (store_contributions) call set_param_value(at, trim(calc_dipole)//"_pot2", dipole)
       dipole = my_dipole_1 + dipole
       call set_param_value(at, trim(calc_dipole), dipole)
    endif
    if (len_trim(calc_local_energy) > 0) then
    ...
    if (len_trim(calc_local_dipole) > 0) then
       if (store_contributions) call add_property(at, trim(calc_local_dipole)//"_pot2", at_local_dipole_ptr, overwrite=.true.)
       at_local_dipole_ptr = my_local_dipole_1 + at_local_dipole_ptr
    end if
    ...
    if (allocated(my_local_dipole_1)) deallocate(my_local_dipole_1)
