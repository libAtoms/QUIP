...
!  call calc_connect(at)
  ...
  do H1=1, at%N
     ...
     !Count the atoms
     call print('Found H'//H1//ElementName(at%Z(H1)),PRINT_INVESTIGATE)
     ...
     do i = 1, n_neighbours(at,H1)
        ...
        call print('HOO_ANGLE '//ElementName(at%Z(H1))//H1//' '//ElementName(at%Z(O1))//O1//' '//ElementName(at%Z(O2))//O2//' '//HOO_angle,PRINT_INVESTIGATE)
        ...
        if (O1>silica_center_i .and. O2>silica_center_i) then  ! water - water
           call print('Found water-water H-bond.',PRINT_INVESTIGATE)
           hbond_type = 1
        elseif (O1>silica_center_i .and. O2<=silica_center_i) then  ! water - silica
           call print('Found water-silica H-bond.',PRINT_INVESTIGATE)
           hbond_type = 2
        elseif (O1<=silica_center_i .and. O2>silica_center_i) then  ! silica - water
           call print('Found silica-water H-bond.',PRINT_INVESTIGATE)
           hbond_type = 3
        elseif (O1<=silica_center_i .and. O2<=silica_center_i) then  ! silica - silica
           call print('Found silica-silica H-bond.',PRINT_INVESTIGATE)
           hbond_type = 4
        endif
...
!  if (gaussian_smoothing) call system_abort('not implemented.')
  ...
  do O=silica_center_i+1, at%N !only check water molecules
     ...
     !Count the atoms
     call print('Found O'//O//ElementName(at%Z(O)),PRINT_INVESTIGATE)
     num_atoms = num_atoms + 1
...
!call print ('angle '//(orientation_angle*180._dp/pi))
     ...
     call print('Storing angle for water '//O//'--'//H1//'--'//H2//' with reference = '//round(orientation_angle,5)//'degrees',PRINT_INVESTIGATE)
     call print('   with distance -1/2 b -- '//O//' = '//round(r,5)//'A',PRINT_INVESTIGATE)
