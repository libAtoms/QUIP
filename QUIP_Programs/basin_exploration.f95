...
do i_cycle=1, n_cycles
  i_sample = int(ran_uniform()*n_samples)+1
  call generate_sample_pos(cur_pos, sample_pos(:, i_sample), gp)
...
subroutine print_gp(gp)
  ...
  do j=0, grid_size
    ...
    write (unit=*, fmt=*) pos, f_predict(gp, pos, SE_kernel_r_rr), f_predict_var(gp, pos, SE_kernel_r_rr)
  end do
  end do
  write (unit=*, fmt='(A)') ""
  write (unit=*, fmt='(A)') ""
