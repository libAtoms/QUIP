...
Refs. \onlinecite{senn_topics_cur_chem_2007a},
\onlinecite{lin_theor_chem_acc_2005a} and \onlinecite{acevedo_acc_chem_res_2010a}.  The vast majority of QM/MM
...
Ref.~\onlinecite{sokol_int_j_quant_chem_2004a} and \onlinecite{sherwood_j_mol_struct_theochem_2003a} for an ionic solid-state
...
not discussed in Ref.~\onlinecite{heyden_j_phys_chem_b_2007a}.   Bulo
...
\item {\bf Interface hybrid code to CP2K}\\
  \begin{itemize}
    \item Developed residue template library that includes all amino acids and nucleic acids in most naturally occurring forms
    \item Developed {\tt xyz2pdb} converter from atomic positions to standard biochemical input format with connectivity, chemical residue/molecule identification
    \item Working interface for {\tt libatoms/QUIP} to CP2K, using the template library to select the appropriate force field
  \end{itemize}
\item {\bf Testing DFT/MM coupling on pure water and on a solvated ion} \\
  Hybrid simulation is stable and shows good agreement for BLYP/TIP3P hybrid method with full BLYP simulation in both cases. 
 \end{enumerate}

The secondary goal of testing with semiempirical QM engines was dropped to focus on DFT, which is more accurate. Once tool chain was in place, two things caused delays, one of them was a bug in dealing with how CP2K requests electrostatics parameters in various modes of its operation, resulting in incorrect forces. The second was that the relaxation of DFT water at 300~K is very slow because it is supercooled.  Once this was realised, the production runs were restarted at a higher temperature. Initially we adjusted the QM region size so that we are  better able to compare with Rode's work, however these simulations confirmed that Rode's method does not work in coupling the pair of models we have chosen. The presently advocated hybrid method does, but we are repeating the runs with a larger QM region to get a more extended match with the fully QM radial distribution function.

The following objectives remain, work for which is currently underway:
\begin{enumerate}
\item Free energy barrier for nucleophilic substitution of methyl chloride (production runs underway)
\item Free energy barrier and difference during deprotonation of an amino acid (production runs about to start)
