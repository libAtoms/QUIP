...
implicit none
  ...
  call Initialise(tbc, "NRL-TB", params, mpi_obj=mpi_glob)
  ...
  call Initialise(tbc, "Bowler", params, mpi_obj=mpi_glob)
  call Print ("Post initialise")
  call Print(tbc)
...
integer i, j
  ...
  call copy (dH0, tbc%tbsys%dH(1)%sdata_z(1))
  ...
  call Initialise(Hp, tbc%tbsys%H%data_z(1)%N)
  Hp%data = tbc%tbsys%H%data_z(1)%data
  ...
  call Initialise(Hm, tbc%tbsys%H%data_z(1)%N)
  Hm%data = tbc%tbsys%H%data_z(1)%data

  fd_f = -(energyp-energym)/(2*dpos)
  write (line, '("force analytical ",f30.20," fd ",f30.20)') forces0(1,fd_ind), fd_f
  call Print(line)

  call Initialise(fd_dH, tbc%tbsys%H%data_z(1)%N)
