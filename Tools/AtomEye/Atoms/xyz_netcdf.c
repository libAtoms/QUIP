...
#ifdef HAVE_NETCDF
#define netcdf_check(s) if ((retval = (s))) pe("NetCDF Error: %s\n", nc_strerror(retval));
#endif
...
void atoms_init(Atoms *atoms) {
  ...
  atoms->filter = NULL;
  atoms->initialised = 0;
  atoms->ncid[NETCDF_IN] = -1;
  atoms->ncid[NETCDF_OUT] = -1;
}
...
int read_netcdf (char *fname, Atoms *atoms, int frame, int *atomlist, int natomlist, int query, 
		 int redefine, int realloc, int replacefill, int irep, double rrep)
{
  ...
  if (atoms->ncid[NETCDF_IN] != -1 && strcmp(fname, atoms->nc_filename[NETCDF_IN]) != 0) {
    netcdf_check(nc_close(atoms->ncid[NETCDF_IN]));
    atoms->ncid[NETCDF_IN] = -1;
  }

  if (atoms->ncid[NETCDF_IN] == -1 || redefine || !atoms->initialised) {
    // Open file and get dimension and variable information
    debug("Opening NetCDF file \"%s\" for reading\n", fname);
    netcdf_check(nc_open(fname, NC_64BIT_OFFSET | NC_NOWRITE, &atoms->ncid[NETCDF_IN]));
    strcpy(atoms->nc_filename[NETCDF_IN], fname);

    // Inquire dimensions
    netcdf_check(nc_inq_dimid(atoms->ncid[NETCDF_IN], "frame", &atoms->frame_dim_id[NETCDF_IN]));
    netcdf_check(nc_inq_dimid(atoms->ncid[NETCDF_IN], "spatial", &atoms->spatial_dim_id[NETCDF_IN]));
    netcdf_check(nc_inq_dimid(atoms->ncid[NETCDF_IN], "atom", &atoms->atom_dim_id[NETCDF_IN]));
    netcdf_check(nc_inq_dimid(atoms->ncid[NETCDF_IN], "cell_spatial", &atoms->cell_spatial_dim_id[NETCDF_IN]));
    netcdf_check(nc_inq_dimid(atoms->ncid[NETCDF_IN], "cell_angular", &atoms->cell_angular_dim_id[NETCDF_IN]));
    netcdf_check(nc_inq_dimid(atoms->ncid[NETCDF_IN], "label", &atoms->label_dim_id[NETCDF_IN]));
    netcdf_check(nc_inq_dimid(atoms->ncid[NETCDF_IN], "string", &atoms->string_dim_id[NETCDF_IN]));

    // Sizes of dimensions
    netcdf_check(nc_inq_dimlen(atoms->ncid[NETCDF_IN], atoms->frame_dim_id[NETCDF_IN], &atoms->n_frame));
    netcdf_check(nc_inq_dimlen(atoms->ncid[NETCDF_IN], atoms->atom_dim_id[NETCDF_IN], &atoms->n_atom_total));
    netcdf_check(nc_inq_dimlen(atoms->ncid[NETCDF_IN], atoms->label_dim_id[NETCDF_IN], &atoms->n_label));
    netcdf_check(nc_inq_dimlen(atoms->ncid[NETCDF_IN], atoms->string_dim_id[NETCDF_IN], &atoms->n_string));
    ...
    netcdf_check(nc_inq_nvars(atoms->ncid[NETCDF_IN], &nvars));
    for (i=0; i<nvars; i++) {
      netcdf_check(nc_inq_var(atoms->ncid[NETCDF_IN], i, varname, &vartype, &ndims, dimids, &natts));
	...
	continue;
      ...
      netcdf_check(nc_get_att_int(atoms->ncid[NETCDF_IN], i, "type", &type));
  ...
  count2[0] = 1;
  count2[1] = 3;
  netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->cell_lengths_var_id[NETCDF_IN], start2, count2, cell_lengths));
  netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->cell_angles_var_id[NETCDF_IN], start2, count2, cell_angles));
  ...
  for (i=0; i<atoms->n_param; i++) {
    ...
    case(T_INTEGER):
      start1[0] = frame;
      netcdf_check(nc_get_var1_int(atoms->ncid[NETCDF_IN], atoms->param_var_id[i][NETCDF_IN], start2, &atoms->param_int[i]));
      break;
    case(T_REAL):
      start1[0] = frame;
      netcdf_check(nc_get_var1_double(atoms->ncid[NETCDF_IN], atoms->param_var_id[i][NETCDF_IN], start2, &atoms->param_real[i]));
      break;
    case(T_CHAR):
      start2[0] = frame;
      start2[1] = 0;
      ...
      netcdf_check(nc_get_vara_text(atoms->ncid[NETCDF_IN], atoms->param_var_id[i][NETCDF_IN], start2, count2, atoms->param_value[i]));
      break;
    case(T_INTEGER_A):
      start2[0] = frame;
      start2[1] = 0;
      count2[0] = 1;
      count2[1] = 3;
      netcdf_check(nc_get_vara_int(atoms->ncid[NETCDF_IN], atoms->param_var_id[i][NETCDF_IN], start2, count2, &atoms->param_int_a[i][0]));
    case(T_REAL_A):
      start2[0] = frame;
      start2[1] = 0;
      count2[0] = 1;
      count2[1] = 3;
      netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->param_var_id[i][NETCDF_IN], start2, count2, &atoms->param_real_a[i][0]));
    default:
  ...
  for (i=0; i<atoms->n_property; i++) {
	...
	//if (atomlist) {
	  netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start2, count2, tmpreal));
	  n = 0;
	  ...
	  //}
	  //else
	  //netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start2, count2, 
...
/* 	  count3[2] = 1; */
/* 	  //if (atomlist) { */
/* 	    netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3, tmpreal)); */
/* 	    n = 0; */
/* 	    for (j=0; j<atoms->n_atom_total; j++) { */
/* 	      if (!atoms->filter[j]) continue; */
...
/* 	    //} */
/* 	    //else */
/* 	    //netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3,  */
...
/* 	    //				    &property_real(atoms, i, k, 0))); */
	...
	count3[1] = atoms->n_atom_total;
	count3[2] = 3;
	netcdf_check(nc_get_vara_double(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3, tmpreal3));
	n = 0;
	for (j=0; j<atoms->n_atom_total; j++) {
	  if (!atoms->filter[j]) continue;
	...
	count2[1] = atoms->n_atom_total;
	//if (atomlist) {
	  netcdf_check(nc_get_vara_int(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start2, count2, tmpint));
	  n = 0;
	  for (j=0; j<atoms->n_atom_total; j++) {
	    if (!atoms->filter[j]) continue;
	  ...
	  //} else 
	  //netcdf_check(nc_get_vara_int(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start2, count2,
...
/* 	  count3[2] = 1; */
/* 	  //if (atomlist) { */
/* 	    netcdf_check(nc_get_vara_int(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3, tmpint)); */
/* 	    n = 0; */
/* 	    for (j=0; j<atoms->n_atom_total; j++) { */
/* 	      if (!atoms->filter[j]) continue; */
...
/* 	    //} */
/* 	    //else */
/* 	    //netcdf_check(nc_get_vara_int(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3,  */
...
/* 	} */
	...
	count3[1] = atoms->n_atom_total;
	count3[2] = 3;
	netcdf_check(nc_get_vara_int(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3, tmpint3));
	n = 0;
	for (j=0; j<atoms->n_atom_total; j++) {
	  if (!atoms->filter[j]) continue;
...
/* 	//				 &property_logical(atoms,i,k,0))); */
	...
	//if (atomlist) {
	  netcdf_check(nc_get_vara_text(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3, tmpchar));
	  n = 0;
	  for (j=0; j<atoms->n_atom_total; j++) {
	    if (!atoms->filter[j]) continue;
	  ...
	  //} else 
	  //netcdf_check(nc_get_vara_text(atoms->ncid[NETCDF_IN], atoms->property_var_id[i][NETCDF_IN], start3, count3,
...
int write_netcdf(char *fname, Atoms *atoms, int frame, int redefine, int append)
{
  int retval;
  int i, j, ndims;
  ...
  // Is this the same file we used last time?
  if (atoms->ncid[NETCDF_OUT] != -1 && strcmp(fname, atoms->nc_filename[NETCDF_OUT]) != 0) {
    netcdf_check(nc_close(atoms->ncid[NETCDF_OUT]));
    atoms->ncid[NETCDF_OUT] = -1;
  }

  if (atoms->ncid[NETCDF_OUT] == -1 || redefine) {
    // Open file and get dimension and variable information
    if (append) {
      debug("Opening NetCDF file \"%s\" for append\n", fname);
      netcdf_check(nc_open(fname, NC_64BIT_OFFSET | NC_WRITE, &atoms->ncid[NETCDF_OUT]));
      newfile = 0;
    } else {
      debug("Opening NetCDF file \"%s\" for writing\n", fname);
      netcdf_check(nc_create(fname, NC_64BIT_OFFSET | NC_CLOBBER, &atoms->ncid[NETCDF_OUT]));
      newfile = 1;
    }
    strcpy(atoms->nc_filename[NETCDF_OUT], fname);

    if (newfile) {
      // Global attributes
      netcdf_check(nc_put_att_text(atoms->ncid[NETCDF_OUT], NC_GLOBAL, "Conventions", strlen("AMBER"), "AMBER"));
      netcdf_check(nc_put_att_text(atoms->ncid[NETCDF_OUT], NC_GLOBAL, "ConventionVersion", strlen("1.0"), "1.0"));
      netcdf_check(nc_put_att_text(atoms->ncid[NETCDF_OUT], NC_GLOBAL, "application", strlen("libAtoms"), "libAtoms"));
      netcdf_check(nc_put_att_text(atoms->ncid[NETCDF_OUT], NC_GLOBAL, "program", strlen("xyz2nc"), "xyz2nc"));
      netcdf_check(nc_put_att_text(atoms->ncid[NETCDF_OUT], NC_GLOBAL, "programVersion", strlen(SVN_VERSION), SVN_VERSION));
      netcdf_check(nc_put_att_text(atoms->ncid[NETCDF_OUT], NC_GLOBAL, "title", strlen("Atoms Object"), "Atoms Object"));

      // Dimensions
      netcdf_check(nc_def_dim(atoms->ncid[NETCDF_OUT], "frame", NC_UNLIMITED, &atoms->frame_dim_id[NETCDF_OUT]));
      netcdf_check(nc_def_dim(atoms->ncid[NETCDF_OUT], "spatial", 3, &atoms->spatial_dim_id[NETCDF_OUT]));
      netcdf_check(nc_def_dim(atoms->ncid[NETCDF_OUT], "atom", atoms->n_atom, &atoms->atom_dim_id[NETCDF_OUT]));
      netcdf_check(nc_def_dim(atoms->ncid[NETCDF_OUT], "cell_spatial", 3, &atoms->cell_spatial_dim_id[NETCDF_OUT]));
      netcdf_check(nc_def_dim(atoms->ncid[NETCDF_OUT], "cell_angular", 3, &atoms->cell_angular_dim_id[NETCDF_OUT]));
      netcdf_check(nc_def_dim(atoms->ncid[NETCDF_OUT], "label", PROPERTY_STRING_LENGTH, &atoms->label_dim_id[NETCDF_OUT]));
      netcdf_check(nc_def_dim(atoms->ncid[NETCDF_OUT], "string", PARAM_STRING_LENGTH, &atoms->string_dim_id[NETCDF_OUT]));
      
      // Label variables
      dims1[0] = atoms->spatial_dim_id[NETCDF_OUT];
      netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], "spatial", NC_CHAR, 1, dims1, &atoms->spatial_var_id[NETCDF_OUT]));

      dims1[0] = atoms->cell_spatial_dim_id[NETCDF_OUT];
      netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], "cell_spatial", NC_CHAR, 1, dims1, &atoms->cell_spatial_var_id[NETCDF_OUT]));

      dims2[0] = atoms->cell_angular_dim_id[NETCDF_OUT];
      dims2[1] = atoms->label_dim_id[NETCDF_OUT];
      netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], "cell_angular", NC_CHAR, 2, dims2, &atoms->cell_angular_var_id[NETCDF_OUT]));

      dims2[0] = atoms->frame_dim_id[NETCDF_OUT];
      dims2[1] = atoms->spatial_dim_id[NETCDF_OUT];
      netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], "cell_lengths", NC_DOUBLE, 2, dims2, &atoms->cell_lengths_var_id[NETCDF_OUT]));
      netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], "cell_angles", NC_DOUBLE, 2, dims2, &atoms->cell_angles_var_id[NETCDF_OUT]));
    }
	...
	if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->param_key[i], &atoms->param_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	  netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->param_key[i], NC_INT, 1, dims1, &atoms->param_var_id[i][NETCDF_OUT]));
	} else {
	  netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	  if (ndims != 1) {
	...
	dims1[0] = atoms->frame_dim_id[NETCDF_OUT];
	if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->param_key[i], &atoms->param_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	  netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->param_key[i], NC_DOUBLE, 1, dims1, &atoms->param_var_id[i][NETCDF_OUT]));
	} else {
	  netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	  if (ndims != 1) {
	    fprintf(stderr,"Mismatch in parameter %s - ndims(%d) != 1\n", atoms->param_key[i], ndims);
	...
	if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->param_key[i], &atoms->param_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	  netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->param_key[i], NC_CHAR, 2, dims2, &atoms->param_var_id[i][NETCDF_OUT]));
	} else {
	  netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	  if (ndims != 2) {
	    fprintf(stderr,"Mismatch in parameter %s - ndims(%d) != 1\n", atoms->param_key[i], ndims);
	...
	dims2[1] = atoms->spatial_dim_id[NETCDF_OUT];
	if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->param_key[i], &atoms->param_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	  netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->param_key[i], NC_INT, 2, dims2, &atoms->param_var_id[i][NETCDF_OUT]));
	} else {
	  netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	  if (ndims != 2) {
	    fprintf(stderr,"Mismatch in parameter %s - ndims(%d) != 1\n", atoms->param_key[i], ndims);
	...
	dims2[1] = atoms->spatial_dim_id[NETCDF_OUT];
	if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->param_key[i], &atoms->param_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	  netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->param_key[i], NC_DOUBLE, 2, dims2, &atoms->param_var_id[i][NETCDF_OUT]));
	} else {
	  netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	  if (ndims != 2) {
	    fprintf(stderr,"Mismatch in parameter %s - ndims(%d) != 1\n", atoms->param_key[i], ndims);
	...
	return 0;
      ...
      nc_put_att_int(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], "type", NC_INT, 1, &(atoms->param_type[i]));
    }
	...
	if (ndims == 1) {
	  ...
	  if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->property_name[i], &atoms->property_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	    netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->property_name[i], NC_INT, 2, dims2, &atoms->property_var_id[i][NETCDF_OUT]));
	  } else {
	    netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	    if (ndims != 2) {
	  ...
	  if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->property_name[i], &atoms->property_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	    netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->property_name[i], NC_INT, 3, dims3, &atoms->property_var_id[i][NETCDF_OUT]));
	  } else {
	    netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	    if (ndims != 3) {
	      fprintf(stderr,"Mismatch in property %s - ndims(%d) != 1\n", atoms->property_name[i], ndims);
	...
	if (ndims == 1) {
	  ...
	  dims2[1] = atoms->atom_dim_id[NETCDF_OUT];
	  if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->property_name[i], &atoms->property_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	    netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], pname, NC_DOUBLE, 2, dims2, &atoms->property_var_id[i][NETCDF_OUT]));
	  } else {
	    netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	    if (ndims != 2) {
	      fprintf(stderr,"Mismatch in property %s - ndims(%d) != 1\n", atoms->property_name[i], ndims);
	  ...
	  dims3[2] = atoms->spatial_dim_id[NETCDF_OUT];
	  if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->property_name[i], &atoms->property_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	    netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], pname, NC_DOUBLE, 3, dims3, &atoms->property_var_id[i][NETCDF_OUT]));
	    debug("%s %d\n", atoms->property_name[i], atoms->property_var_id[i][NETCDF_OUT]);
	  } else {
	    netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	    if (ndims != 3) {
	      fprintf(stderr,"Mismatch in property %s - ndims(%d) != 1\n", atoms->property_name[i], ndims);
	...
	if (ndims == 1) {
	  ...
	  if (nc_inq_varid(atoms->ncid[NETCDF_OUT], atoms->property_name[i], &atoms->property_var_id[i][NETCDF_OUT]) != NC_NOERR) {
	    netcdf_check(nc_def_var(atoms->ncid[NETCDF_OUT], atoms->property_name[i], NC_CHAR, 3, dims3, &atoms->property_var_id[i][NETCDF_OUT]));
	  } else {
	    netcdf_check(nc_inq_var(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], varname, &vartype, &ndims, dimids, &natts));
	    if (ndims != 3) {
	      fprintf(stderr,"Mismatch in property %s - ndims(%d) != 1\n", atoms->property_name[i], ndims);
	    ...
	    if (dimids[0] != atoms->frame_dim_id[NETCDF_OUT] || dimids[1] != atoms->atom_dim_id[NETCDF_OUT] || dimids[2] != atoms->label_dim_id[NETCDF_OUT])  {
	      fprintf(stderr,"Mismatch in property %s - dimension 0 != frame or dimension 1 != atom or dimension 2 != label\n", 
		      atoms->property_name[i]);
	      return 0;
	    }
	  }	
	...
	break;
      ...
      nc_put_att_int(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], "type", NC_INT, 1, &(atoms->property_type[i]));
    }
      
    netcdf_check(nc_enddef(atoms->ncid[NETCDF_OUT]));

    if (newfile) {
      // Put data for label variables
      netcdf_check(nc_put_var_text(atoms->ncid[NETCDF_OUT], atoms->spatial_var_id[NETCDF_OUT], "xyz"));
      netcdf_check(nc_put_var_text(atoms->ncid[NETCDF_OUT], atoms->cell_spatial_var_id[NETCDF_OUT], "abc"));
    
      start2[0] = 0;
      start2[1] = 0;
      count2[0] = 1;
      count2[1] = strlen("alpha");
      netcdf_check(nc_put_vara_text(atoms->ncid[NETCDF_OUT], atoms->cell_angular_var_id[NETCDF_OUT], start2, count2, "alpha"));
      start2[0] = 1;
      start2[1] = 0;
      count2[0] = 1;
      count2[1] = strlen("beta");
      netcdf_check(nc_put_vara_text(atoms->ncid[NETCDF_OUT], atoms->cell_angular_var_id[NETCDF_OUT], start2, count2, "beta"));
      start2[0] = 2;
      start2[1] = 0;
      count2[0] = 1;
      count2[1] = strlen("gamma");
      netcdf_check(nc_put_vara_text(atoms->ncid[NETCDF_OUT], atoms->cell_angular_var_id[NETCDF_OUT], start2, count2, "gamma"));
    }

  } /* end if (atoms->ncid[NETCDF_OUT] == -1) */
  ...
  count2[0] = 1;
  count2[1] = 3;
  netcdf_check(nc_put_vara_double(atoms->ncid[NETCDF_OUT], atoms->cell_lengths_var_id[NETCDF_OUT], start2, count2, cell_lengths));
  netcdf_check(nc_put_vara_double(atoms->ncid[NETCDF_OUT], atoms->cell_angles_var_id[NETCDF_OUT], start2, count2, cell_angles));
  ...
  for (i=0; i<atoms->n_param; i++) {
	...
	start1[0] = frame;
	count1[0] = 1;
	netcdf_check(nc_put_vara_int(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], start1, count1, &atoms->param_int[i]));
	break;
      case(T_REAL):
	start1[0] = frame;
	count1[0] = 1;
	netcdf_check(nc_put_vara_double(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], start1, count1, &atoms->param_real[i]));
	break;
      case(T_CHAR):
	start2[0] = frame;
	start2[1] = 0;
	count2[0] = 1;
	count2[1] = PARAM_STRING_LENGTH;
	netcdf_check(nc_put_vara_text(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], start2, count2, atoms->param_value[i]));
	break;
      case(T_INTEGER_A):
	start2[0] = frame;
	start2[1] = 0;
	count2[0] = 1;
	count2[1] = 3;
	netcdf_check(nc_put_vara_int(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], start2, count2, &atoms->param_int_a[i][0]));
	break;
      case(T_REAL_A):
	start2[0] = frame;
	start2[1] = 0;
	count2[0] = 1;
	count2[1] = 3;
	netcdf_check(nc_put_vara_double(atoms->ncid[NETCDF_OUT], atoms->param_var_id[i][NETCDF_OUT], start2, count2, &atoms->param_real_a[i][0]));
	break;
      default:
	  fprintf(stderr,"Unknown parameter %s type %d when writing netcdf file\n", atoms->param_key[i],
		  atoms->param_type[i]);
  ...
  for (i=0; i<atoms->n_property; i++) {
	...
	//netcdf_check(nc_put_vara_int(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], start2, count2, 
	//			     &atoms->int_data[(atoms->property_start[i])*atoms->n_atom]));	
...
/* 	  count3[2] = 1; */
/* 	  //netcdf_check(nc_put_vara_int(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], start3, count3,  */
/* 	  //			       &atoms->int_data[(atoms->property_start[i] + k)*atoms->n_atom])); */
/* 	  for (j=0; j<atoms->n_atom; j++) */
...
/* 	} */
	...
	netcdf_check(nc_put_vara_int(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], 
	...
	count2[0] = 1;
	count2[1] = atoms->n_atom;
	//netcdf_check(nc_put_vara_double(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], start2, count2, 
	//				&atoms->real_data[atoms->property_start[i]*atoms->n_atom]));
	for (j=0; j<atoms->n_atom; j++)
...
/* 	  count3[1] = atoms->n_atom; */
/* 	  count3[2] = 1; */
/* 	  //netcdf_check(nc_put_vara_double(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], start3, count3,  */
/* 	  //				  &atoms->real_data[(atoms->property_start[i] + k)*atoms->n_atom])); */
/* 	  for (j=0; j<atoms->n_atom; j++) */
...
/* 	} */
	...
	netcdf_check(nc_put_vara_double(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], 
    ...
    case(PROPERTY_STR):
      ...
      count3[1] = atoms->n_atom;
      count3[2] = PROPERTY_STRING_LENGTH;
      //   netcdf_check(nc_put_vara_text(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], start3, count3,
      //			    &(atoms->str_data[PROPERTY_STRING_LENGTH*atoms->property_start[i]*atoms->n_atom])));
      for (j=0; j<atoms->n_atom; j++)
...
/* 	  count3[1] = atoms->n_atom; */
/* 	  count3[2] = 1; */
/* 	  //netcdf_check(nc_get_vara_int(atoms->ncid[NETCDF_OUT], atoms->property_var_id[i][NETCDF_OUT], start3, count3,  */
/* 	  //			       &atoms->logical_data[(atoms->property_start[i] + k)*atoms->n_atom])); */
/* 	  for (j=0; j<atoms->n_atom; j++) */
...
/* 	} */
  ...
  netcdf_check(nc_sync(atoms->ncid[NETCDF_OUT]));
  return 1;
}
...
int read_xyz (FILE *in, Atoms *atoms, int *atomlist, int natomlist, int frame, 
	      int query, int redefine, int realloc) {
  ...
  int properties_idx, lattice_idx, nxyz, original_index=0, nfields, idx;
  ...
  if (!fgets(linebuffer,LINESIZE,in)) {
    fprintf(stderr, "premature file ending - expecting number of atoms\n");
    return 0;
  }
...
int write_xyz(FILE *out, Atoms *atoms, char *int_format, char *real_format, char *str_format, char *logical_format, int swap) {
  ...
  for (n=0; n<atoms->n_atom; n++) {
	...
	for (j=0; j < atoms->property_ncols[i]; j++) {
	  ...
	  sprintf(linebuffer, logf, linebuffer, property_logical(atoms, i, j, n) ? 'T' : 'F');
	}
	break;
      }
    }
...
int main (int argc, char **argv) {
  ...
  int ch, cflag, rflag, nflag, aflag, oflag, fflag, pflag, Pflag, Lflag, dflag, zflag;
...
if (!(xyzstat || ncstat)) {
  ...
  if (xyz2xyz || xyz2nc || xyzstat) {
    ...
    if (xyzstat && (pflag || Pflag)) {
      if (!read_xyz(infile, &at, atomlist, natomlist, 0, 1, 0, 1))
	...
	for (i=f_start; i < f_stop; i += f_step) {
	  if (!read_xyz(infile, &at, atomlist, natomlist, i, 0, 0, 1)) 
	  ...
	  if (xyz2xyz || xyz2nc) {
		...
		pe("Error writing xyz\n");
	      ...
	      if (!write_netcdf(outfilename, &at, n++, 0, 0))
		pe("Error writing netcdf\n");
	    }

	    fprintf(stderr, "Frame %d/%d                      %c", i, nframes, i + f_step >= f_stop ? '\n' : '\r');
	  } else {
	...
	while ((res = read_xyz(infile, &at, atomlist, natomlist, 0, 0, 0, 1))) {
	  debug("read frame %d\n", n);
...
#ifdef HAVE_NETCDF
    if (xyz2nc) netcdf_check(nc_close(at.ncid[NETCDF_OUT]));
#endif
    ...
    // Read NetCDF header
    nframes = read_netcdf(infilename, &at, 0, atomlist, natomlist, 1, 0, 1, zflag, 0, 0.0);
    ...
    n = 0;
    for (i = f_start; i < f_stop; i += f_step) {
      if (!read_netcdf(infilename, &at, i, atomlist, natomlist, 0, 0, 1, zflag, 0, 0.0))
...
#ifdef HAVE_NETCF
    netcdf_check(nc_close(at.ncid[NETCDF_IN]));
    if (nc2xyz && strcmp(outfilename, "-") != 0) fclose(outfile);
    if (nc2nc) netcdf_check(nc_close(at.ncid[NETCDF_OUT]));
#endif
  } 
	     ...
	     int **param_int, double **param_real, int **param_int_a, double **param_real_a, int **param_filter, double **lattice)
{
  ...
  *n_property = &((**at).n_property);
  ...
  (**at).xyz_in = NULL;
  (**at).xyz_out = NULL;
  (**at).nc_filename[NETCDF_IN][0] = '\0';
  (**at).nc_filename[NETCDF_OUT][0] = '\0';
  (**at).append = *append;
  ...
  NETCDF:
    if (*action == INPUT)
      strcpy((**at).nc_filename[NETCDF_IN], filename);
    else if (*action == OUTPUT)
      strcpy((**at).nc_filename[NETCDF_OUT], filename);
    else if (*action == INOUT) {
      strcpy((**at).nc_filename[NETCDF_IN], filename);
      strcpy((**at).nc_filename[NETCDF_OUT], filename);
    }
...
void cio_free(Atoms *at)
{
  ...
  if (at->format == XYZ_FORMAT) {
    if (at->xyz_in  != NULL)   fclose(at->xyz_in);
    if (at->xyz_out != NULL && at->xyz_out != stdout) fclose(at->xyz_out);
  } else if (at->format == NETCDF_FORMAT) {
#ifdef HAVE_NETCDF
    if (at->nc_filename[NETCDF_IN][0]  != '\0') netcdf_check(nc_close(at->ncid[NETCDF_IN]));
    if (at->nc_filename[NETCDF_OUT][0] != '\0') netcdf_check(nc_close(at->ncid[NETCDF_OUT]));
#else
    pe("No NetCDF support compiled in");
#endif
  }
...
int cio_query(Atoms *at, int *frame) {
  if (at->format == XYZ_FORMAT) {
    if (at->xyz_in == NULL) return 0;
    return read_xyz((*at).xyz_in, at, NULL, 0, *frame, 1, 1, 0);
  } else if (at->format == NETCDF_FORMAT) {
    if (at->nc_filename[NETCDF_IN][0] == '\0') return 0;
    return read_netcdf((*at).nc_filename[NETCDF_IN], at, *frame, NULL, 0, 1, 1, 0, 0, 0, 0.0);
  }
}
	     ...
	     int *logical_data, int *zero)
{
  ...
  if (at->format == XYZ_FORMAT) {
    if (at->xyz_in == NULL) return 0;
    status = read_xyz(at->xyz_in, at, NULL, 0, *frame, 0, 0, 0);
    if (status == 0) return status;
    return status;
  } else if (at->format == NETCDF_FORMAT) {
    if (at->nc_filename[NETCDF_IN][0] == '\0') return 0;
    return read_netcdf(at->nc_filename[NETCDF_IN], at, *frame, NULL, 0, 0, 0, 0, *zero, 0, 0.0);
  }
}

int cio_write(Atoms *at, int *int_data, double *real_data, char *str_data, int *logical_data,
	      char *intformat, char *realformat, int *frame)
{
  at->int_data = int_data;
  at->real_data = real_data;
  at->str_data = str_data;
  ...
  if (at->format == XYZ_FORMAT) {
    ...
    if (!write_xyz(at->xyz_out, at, intformat, realformat, "%.10s", "%5s", 0))
      return 0;
    if (at->xyz_out == stdout) fflush(at->xyz_out);
    return 1;
  } else if (at->format == NETCDF_FORMAT) {
    if (at->nc_filename[NETCDF_OUT] == NULL) return 0;
    return write_netcdf(at->nc_filename[NETCDF_OUT], at, *frame, 0, at->append);
  }
}
