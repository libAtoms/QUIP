      DOUBLE PRECISION FUNCTION SIMANNF77 (FE,N,T,RT,ISEED,L,U,GUESS,X)
      PARAMETER (MAXN=1024,NEPS=4)
      DOUBLE PRECISION  L(MAXN), U(MAXN), GUESS(MAXN), X(MAXN), C(MAXN), 
     1         VM(MAXN), FSTAR(NEPS), XP(MAXN), T, EPS, RT, FOPT, FE
      EXTERNAL FE
      INTEGER  NACP(MAXN), NS, NT, NFCNEV, IER, ISEED1, ISEED2,
     1         MAXEVL, IPRINT, NACC, NOBDS
      LOGICAL  MAX
C     T = 5.0
C     RT = .85
C     IF ((5*N).GE.100) THEN
C     NT = 5*N
C     ELSE
C     NT = 100
C     ENDIF
...
10    CONTINUE
      ...
      VM(I) = 1.0
 20   CONTINUE
      CALL SA(FE,N,GUESS,MAX,RT,EPS,NS,NT,NEPS,MAXEVL,L,U,C,IPRINT,
     1        ISEED1,ISEED2,T,VM,X,FOPT,NACC,NFCNEV,NOBDS,IER,
     2        FSTAR,XP,NACP)
      SIMANNF77 = FOPT
      RETURN
      END
