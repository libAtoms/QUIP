...
try:
   import ase
   from quippy.aseinterface import *
   got_ase = True
except ImportError:
   got_ase = False
...
if got_ase:
   ...
   class TestConvert_Simple(QuippyTestCase):
      ...
      def setUp(self):
         ...
         self.dia_ase      = atoms_to_ase(self.dia_quippy)
         self.dia_quippy_2 = atoms_from_ase(self.dia_ase)
   ...
   class TestConvert_ExtraProperties(QuippyTestCase):
      ...
      def setUp(self):
         self.dia_quippy   = diamond(5.44, 14)
         self.dia_quippy.add_property('mass', [ElementMass[z] for z in self.dia_quippy.z])
         self.dia_quippy.add_property('velo', numpy.random.uniform(-1,1,size=3*8).reshape(3,8))
