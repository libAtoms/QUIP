...
def find_wrap_sources(makefile, quip_root):
    ...
    if do_tools:
        wrap_sources += [os.path.join(quip_utils_dir, s) for s in ['elasticity.f95', 'real_space_covariance.f95', 'ts_params.f95', 'transition_state.f95']]
...
setup(name='quippy',
      ...
      version=os.popen('%s/utility_scripts/gitversion -n .' % quip_root).read(),
